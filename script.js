...
function questionSuivante() {
  ...
  if (genreDesNoms.length == 0) {
    alert("Il ne reste plus de mot dans la liste. Veuilez choisir une autre liste");
    return;
  }
...
function changerDeListe() {
  var reponse = prompt("Quelle liste de mot voulez-vous ? (informatique(i), maison(m), automobile(a))");
  console.log(reponse);
  switch (reponse) {
    case "i":
      genreDesNoms = [];
...
function pasPret() {
  ...
  document.querySelector("p").innerHTML = "Une seule chose peut-être dit, si le mot fini par un 'e' il y a de bonne chance que ça soit féminin. C'est le temps de monter sur le ring !";
  document.querySelector("p").style.color = "red";
  //effacer le bouton
  document.getElementsByTagName("a")[1].hidden = true;
  //Change le bouton
  document.querySelector(".btn").classList.remove("btn-primary");
